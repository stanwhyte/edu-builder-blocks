...
display_usage() { 
  echo -e "\nUsage:\ncreate-appstream-directory.sh -e DEV\n" 
  exit 0
} 
...
echo ""
echo "Creating the Active Directory Appstream Directory Config"
echo "--------------------------------------------------------"
directoryresults=$(aws appstream create-directory-config --directory-name $name --organizational-unit-distinguished-names ou=Appstream,dc=ad,dc=awsexpo,dc=com --service-account-credentials AccountName=$shortname\\$adminid,AccountPassword=$adminpassword)

echo "Successfully created appstream directory config with results $directoryresults"
